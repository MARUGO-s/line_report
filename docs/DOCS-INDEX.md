...
- [利用状況の集計元・LINE公式利用量](./USAGE-METRICS.md): DB/public/Storageの違い、配信記録と利用枠、未確認/失敗の扱い。
...
| [SUPABASE-OWNERSHIP.md](./SUPABASE-OWNERSHIP.md) | 開発・運用 | 同一Supabaseプロジェクト内のLINE Report所有Function・Journalテーブル・Storage・共有リスク境界 |
...
| [JOURNAL-KNOWLEDGE-MENU-PROMPT-BLOCKS.md](./JOURNAL-KNOWLEDGE-MENU-PROMPT-BLOCKS.md) | AI・開発 | 店舗資料メニュー画像の共通・種別・店舗専用プロンプト設計。小口レシートと同じ加算式ブロック構造、MARUGO S専用規約、品質ゲート |
...
| [フードコート学習システム構造.md](./フードコート学習システム構造.md) | 開発・運用 | 来客予測の現行フロー（完了実績→5方式発行→台帳→同条件評価→週次採用判定） |
| [FOODCOURT-FORECAST-AUDIT.md](./FOODCOURT-FORECAST-AUDIT.md) | 開発・運用 | **数値予測の現行正本** — 5方式・改変不可の事前予測台帳・先行日数別WAPE/MAE・週次採用条件 |
| [FOODCOURT-AI-RELIABILITY.md](./FOODCOURT-AI-RELIABILITY.md) | 開発・運用 | **文章分析AI・画像抽出の現行構成** — 反証Gemini・評価Groq、時間配分・一時HTTP再試行・全滅の原因分類 |
| [フードコート来客予測モデル.md](./フードコート来客予測モデル.md) | 開発・運用 | 現行5方式の入口と旧方式との差分 |
| [フードコート来客予測モデル_旧方式履歴.md](./フードコート来客予測モデル_旧方式履歴.md) | 履歴 | 廃止済みの乗算/GLM・MAPE選択の数式と設計記録。現行実装に使わない |
...
| [README.md](../README.md) | 運用・開発 | LINE Reportの入口。画面・構成・予測機能・開発/デプロイと詳細文書へのリンク |
...
*最終更新: 2026-09-10（数値予測の現行仕様・旧方式履歴を整理。Journal予測・生成AI品質との区別を追記。）*
