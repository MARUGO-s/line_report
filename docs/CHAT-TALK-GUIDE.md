...
マニュアルには「M-talkの仕組み・全体像」セクションがあり、ルーム種別・店舗Bot・
2種類のAI（雑談AI／ジャーナルに聞く）・Web Push・画像の非公開保存・参加時点以降の
履歴・4種の権限を、利用者向けにまとめている。`isMtalkHelpQuestion` は「仕組み・とは・
違い・なぜ・安全・プライバシー・対応」なども使い方質問として扱う。`buildMtalkHelpReference`
は、どの質問でも「全体概要＋仕組み・全体像＋関連する詳細＋全機能の索引」を渡すため、
別の機能を続けて聞かれても同じ抜粋の索引から案内できる。
