...
**1対1トーク専用の別画面**。`mtalk_journal_ai.html` を `＋` →「ジャーナルに聞く」
から開く。グループではボタン自体を出さない（売上の質問と回答が参加者全員に
見えてしまうため）。

`mtalk_schedule.html` と同じ型で、チャット利用者の Supabase JWT をそのまま使う。
`admin-api` の `/chat-journal-ai` が JWT を検証し、参加者であること・1対1で
あることを確かめてから `askPosJournalAi` を呼ぶ。管理セッションは要らない。

    GET  /chat-journal-ai?group_id=   これまでのやり取り
    POST /chat-journal-ai             { group_id, month, question }

### 画面でできること

- 対象月をセレクタで選ぶ（質問のたびに月を書かなくてよい）
- スレッド表示で続きの質問ができる。直近8往復を `history` として渡すので
  「じゃあ前月は？」が通る
- 回答ごとに「**この回答をトークに貼る**」。画面に閉じたままだと共有できないため
- 「AI生成」／「基本回答（AI未使用）」とモデル名を出す。`warning` は赤字。
  区別せず出すと「AIが分析した」と誤解される

### PDF出力

「PDF」ボタンで、そのときのやり取りを Q&A 形式の1枚にまとめて出す。
既存のジャーナル画面と同じく **`window.print()`** を使い、印刷ダイアログの
「PDFとして保存」で保存する。PDFライブラリは入れない。

質問と回答を1往復ずつ組にし、回答が付いていない質問も「（回答なし）」として
落とさずに出す。「AI生成／基本回答」の表記と `warning` も帳票へ持っていく
（画面で見えていた但し書きが、配った先で消えないように）。

会話は `mtalk_journal_qa_history` に持つ。質問文と回答が入るので service_role
専用。トリガで1対1×利用者ごとに直近32行へ刈る。

`summary` は渡さない。未指定なら `resolvePosJournalAiSummary` が DB から同じ
ものを組み直すため、画面の状態に依存しないぶん確実。

> **チャット内での一問一答は廃止した。** 以前は「ジャーナル検索」→2分以内に
> 「月6桁＋質問」を送る作りだったが、コマンドと長い回答が普通の会話に混ざり、
> 保留が切れると入力がただの発言になった。同じことをする道が2つあると片方
> だけ直す事故が起きるので、画面へ一本化している。

> ⚠️ **特記事項: Bistro CAVACAVA 専用（2026-08-25 時点）**
>
> `resolvePosJournalAiStore()` が `resolvePosJournalStore("1015")` と一致しない
> 店舗を明示的に弾く。これは意図的な制約で、`.lzh` 取込の店舗コード登録とは別。
