...
M-talkの通常メッセージはMarkdownを描画しないプレーンテキスト表示なので、
雑談AIにはMarkdown記法を使わず、箇条書きを「・」、小見出しを「■」にするよう
指示する。モデルが `**強調**`、`-` 箇条書き、`##` 見出し、表などを返した場合も、
`formatCasualReplyForMtalk()` が保存直前に読みやすいプレーンテキストへ整形する。
HTMLへ変換せず、従来どおり `chat_messages.kind='text'` として保存・表示する。

読みやすさのため、`formatCasualReplyForMtalk()` はトップレベル項目を
「▶ 見出し」＋説明行へ分け、項目ごとに空行を1行入れて区切る。子項目は全角
スペースで字下げした「・」にし、表は「項目：値」、水平線は区切り線へ整形する。
AIにも「- 項目名：説明」で1項目ずつ書き、項目間へ空行を入れるよう指示する。
...
### M-talk／LINE Report／Journal Report統合マニュアルの参照

雑談AIは、M-talk・LINE Report・Journal Reportの機能、操作、仕組み、違いを
自然文で質問された場合、次の統合資料を参照する。

- M-talk: `supabase/functions/_shared/mtalk_help_manual.ts`
...
- 資料は詳細に保つが、回答では最初に結論1〜2文、その後に必要な手順・理由・注意点だけを示す
- 関係のない機能や索引全体を回答へ並べない
- ボタン名と操作場所を具体的に説明する
- マニュアルにない機能・場所・手順は推測しない
- LINEアプリの使い方とM-talkの使い方を混同しない
- LINE Report売上分析とJournal Report電子ジャーナル分析は、入口・数値の正本・用途を分けて説明する
- 通常の雑談にはマニュアルを注入しない
- 売上・客数などの数値質問には答えず、「入力欄の『＋』→『ジャーナルに聞く』を開いて確認してください」と開く場所まで添えて案内する

M-talk資料には「M-talkの仕組み・全体像」セクションがあり、ルーム種別・店舗Bot・
2種類のAI（雑談AI／ジャーナルに聞く）・Web Push・画像の非公開保存・参加時点以降の
...
検索精度は項目名の完全一致だけでなく、省略・否定形・単語が離れた表現・類似機能の
比較もテストする。実利用を模した40問の敵対的検証で見つかったBot削除、過去AI文章、
Journal天候、小ホール係数、LINE Bot数、外部検索条件の検索漏れを修正し、40/40件が
関連項目へ到達し、参照資料が途中で切れないことを確認している。
