...
- 予約タブは Gmail 予約取り込み（食べログ／一休／手入力）と同じ店舗データを表示する。
- 予定タブは、その M-talk ルームと、同じ店舗キーの LINE ルームに登録されたカレンダー予定を表示する。
- 予約の時刻・氏名・人数はオレンジ。アレルギーがあるときは赤、お誕生日／記念日／お祝いがあるときはピンクで別表示する。
- 閲覧はルームメンバーのログイン JWT のみ。管理トークンは使わない。予約の生 JSON（`reservation_detail`）は返さない。
