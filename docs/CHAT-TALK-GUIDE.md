...
**1対1のトーク専用。** グループでは受けない（売上の質問と回答が参加者全員に
見えてしまうため）。合図を受けた時点と、質問を受けた時点の両方で `is_direct`
を確認する。

「**ジャーナル検索**」（または `srch=jnl` ボタン／「電子ジャーナルに聞く」
...
画面のQ&Aと同じく、直近の往復を `history` として渡すので

    202608 客単価は？
    → 回答
    じゃあ前月は？

が通る。往復は `mtalk_journal_qa_history` に持ち、**直近8往復かつ2時間以内**
だけを参照する（古い文脈が別件の質問へ混ざらないため）。行はトリガで
1対1×利用者ごとに直近32行へ刈る。質問文と回答が入るので service_role 専用。
...
`ai_generated` が false のとき、AI ではなく定型の**基本回答**が返っている。
カードに「AI生成」／「基本回答（AI未使用）」とモデル名を出し、`warning` が
あれば赤字で添える。区別せず出すと「AIが分析した」と誤解される。
