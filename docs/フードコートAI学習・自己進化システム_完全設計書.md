...
| 反証AI | 捏造、因果断定、日付ずれ、根拠不足の監査 | Moonshot `kimi-k3` → Claude `claude-haiku-4-5` | 3専門メモと計算済み根拠 |
...
専門AI①②③は失敗時にGroqへフォールバックする。反証AIはKimi K3からClaude Haikuへフォールバックする。OpenAI経路のみ、OpenAI、Gemini、Groqの順で試す。評価AIはClaude既定を維持する。
