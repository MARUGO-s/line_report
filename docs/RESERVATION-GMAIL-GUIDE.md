...
- 通常の売上分析: `summary_facts`（予約数・人数・新規/リピート・月次）だけを取得
- 予約者名・予約一覧・前回来店・顧客等の質問: 必要時だけ氏名入り`facts.items`を取得
...
- 新規/リピートは、予約当時の同店舗における食べログ・一休・manual横断回数で判定
- 前回来店相当は、その予約より前の同店舗・同顧客の直前予約
- 取得は1,000件ずつページング。安全上限超過時は不完全集計を返さずエラー
- `reservation_ai_cache_runs`に成功/失敗・件数・所要時間を記録
- 最終成功から36時間超の場合は古いキャッシュを使わず予約DBへフォールバック
