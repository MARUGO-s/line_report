...
#### AI 呼び出し回数（例：クラウディア2＝Claude 解析店）

| ケース | ① Groqレシート判定 | ② 精算解析 | ③ 経費再解析 | 合計 |
|--------|:---:|:---:|:---:|:---:|
| 通常の売上レシート（自店一致） | ✓(Groq) | ✓(Claude) | – | Groq1+Claude1 |
| 経費（B/C：店名不一致・レジ出金） | ✓(Groq) | ✓(Claude) | ✓(Claude) | Groq1+Claude2 |
| 経費（A：先打ち・最適化後） | – | **省略** | ✓(Claude) | Claude1 |

> Groq は安価な「レシートか否か」の事前判定。経費再解析(`reanalyzeAsExpense`)は Claude キーがあれば Claude、無ければ Groq。Gemini 解析店（ソバージュ・鮨こるり）は ② が Gemini。
...
- **不変**: **先打ちしない通常アップロードは従来どおり**（精算解析 → 必要時のみ経費再解析）。店名不一致(C)は精算解析の結果で初めて判明するため、その経路は精算解析を省けません（仕組み上2回必要）。

> **運用推奨**: 経費レシートは **先に「経費」と送ってから画像** を送る運用にすると、経費1枚ごとに Claude 呼び出しを1回節約できます。

### 品目モデル（勘定科目・税率は品目ごと）

`petty_cash_entries.items`(jsonb) = `[{ n:品目, p:税抜価格, acct:'shokuzai'|'shomohin'|'alcohol', rate:8|10 }]`。1枚のレシートで 食材(8%)＋消耗品(10%)＋アルコール(10%) の混在に対応。金額の権威は `amount_yen`(税込合計)/`tax_yen`(税額)＝本体 `Σp`、税 `Σ round(p×rate/100)`（品目ごとに丸めて合算）、出金額＝本体+税。
...
- **項目別CSVダウンロード**（Excel対応・UTF-8 BOM付き・カンマ/引用符/改行エスケープ・数値は素のまま）。
...
- ワンタイムログイン(`lt`)は売上分析と同じ仕組み。本部で店舗が固定されてしまう場合は `localStorage`/`sessionStorage` の `line_locked_store` を消すと解除。
