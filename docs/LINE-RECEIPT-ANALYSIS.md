...
修正可能項目: 店名、日付、純売上、消費税、総売上、会計組数、客数、客単価。番号で項目を選び値を入力する操作を**複数回繰り返せます**（例: `5` → 金額 → `6` → 組数、または `5 140000` / `6 3`）。変更済み項目は ✓ 表示。すべて終わったら「すべて保存して終了」（テキスト「確定」も可）で保存します。
...
この形式では、**発送元と品名を別々に読みます**。

| 保存項目 | 読取場所 |
|---|---|
| 仕入先 `store_name` | 「発送元」欄の会社名 |
| 品名 `line_items[].name` | 「品名」欄の具体的な商品名 |
| 支払総額 `gross_sales` | 右上「代金引換額（税込）」の枠内 |
| 税率 | 品名が飲食料品なら8%、食品でないことが明確なら10% |

画像例は、仕入先「木次乳業有限会社」、品名「木次パスチャライズ牛乳 1000ml」、税込金額¥5,670です。宛先の店舗名、下部の「ヤマト運輸株式会社」、併写された店舗POSのレジ出金伝票は、店名・品名の根拠にしません。

横長形式へ一致した場合は、従来ヤマトブロックの「品名＝発送元」規則を適用しません。従来の「ご依頼主」型は既存ルールを維持します。
