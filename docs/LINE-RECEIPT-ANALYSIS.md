...
| 経費（店名不一致・レジ出金） | ✓(Groq) | ✓(Claude) | Groq1+Claude1 |
| 経費（A：先打ち） | **省略** | ✓(Claude) | Claude1 |

> 売上(精算)は大多数の店で **Groq**（ソバージュ・鮨こるりは Gemini）。経費の明細再解析(`reanalyzeAsExpense`)だけ **Claude**（キーが無ければGroq）。
> ※以前 claudia2 を「Claude で売上解析する店」にしていたが、**2026-06-12 にコスト優先で Claude→Groq へ変更**（`CLAUDE_RECEIPT_STORE_KEYS` を空に）。これにより Claude売上解析の経路（および下記オプションB最適化）は **現在休止**（売上解析にClaudeを使う店がゼロのため）。経費の明細解析は引き続きClaude。
