...
2. 初回のみ表示名とアイコンを決める。
3. プロフィール作成時に表示名・所属店舗（必須・複数可）を決める。`chat_user_access` は**承認待ち**（`signup_status=pending`、`access_enabled=false`）。
4. 許可／不許可カードは、本部から「新規登録・所属店舗の変更を承認できる」を明示付与された人だけの「管理者通知」ルームに届く。通常ルームを管理できても承認者にはならない。許可されると申請した所属店舗が付き、閲覧のみで始まる。
5. 所属店舗はプロフィール設定から後で変えられる。変更も管理者の許可後に反映する。新しい1対1は所属が重なる相手だけ。

> 本部管理者は後から、M-talk だけの利用停止・一時制限・論理削除を行える。

ログイン画面の保存チェックはメールアドレスだけを端末へ保存する。パスワードは保存せず、
旧版が保存していたパスワードも初回起動時に削除する。共用端末ではチェックを外す。
...
「普通のトーク画面からBotを消せる？」「過去のAI文章を次の回答に使う？」「小ホールも
ドームと同じ係数？」のような、正式名称を使わない質問も検索テストへ含める。類似・否定形
40問を通し、関連資料の選択と途中切れの有無を確認している。
...
通常会話では答えず、利用権限がある人には「ジャーナルに聞く」を案内する。

天気の質問には店舗のおおまかな地域の予報で答える。ルーム設定で
「AIのWeb検索」がオンのときだけ、調べ物にWebを使う（既定オフ・従量課金）。

「M-talkの仕組みは？」「雑談AIとジャーナルに聞くの違いは？」「なぜ参加前の
メッセージは見えないの？」のような仕組み・全体像の質問にも答えられるよう、
マニュアルに「M-talkの仕組み・全体像」を用意している。回答には毎回、全体像と
全機能の索引が渡されるため、続けて別の機能を聞いても同じ場で案内できる。

AIの返信はM-talkの吹き出しでそのまま読みやすいプレーンテキストに整える。
Markdownの `**`、`##`、`-`、表などは表示せず、箇条書きは「・」、
小見出しは「■」として表示する。
項目が多いときは、各項目を「▶ 見出し」と説明に分け、項目の間に空行を入れて
詰まらずに読めるよう整形する。

「ジャーナルに聞く」の回答を「M-talkに貼る」で貼る際、本文が2000文字を
超える場合は自動的に複数メッセージへ分割する。途中で切り捨てず、2件目以降は
「[電子ジャーナル]（続き N/全件数）」として全文を順番に貼る。貼り付けた各断片には
店舗Bot・雑談AIは返信しない。

「ジャーナルに聞く」は、人間が本人だけで店舗Botがいる1対1、かつ本部がその本人へ
電子ジャーナルAI権限を付与した場合だけ表示する。専用ログインは店舗・ルーム・本人に
固定した30分セッションで、売上・商品分析以外の予約者情報、資料、店舗設定、取込、
削除操作には使えない。利用停止、権限取消、ルーム退出、店舗所属取消、店舗Bot差替えは
次のAPI操作から直ちに効く。

営業分析では、Journalでクラウド保存した営業ルール・共有店舗メモ・対象期間の施策カレンダー・ワイン換算設定も、サーバーが同じ店舗の情報を毎回確認して使う。送信先OpenAI／Claudeへの明示承認済みで、電話・メール等の送信前処理を行う。個人メモや他店、設定変更権限は追加しない。回答末尾に登録状態と更新日時を表示し、共有情報の取得失敗は古い端末値へ戻らず停止する。詳細は[3アプリ連携監査](./THREE-APP-DATA-INTEGRATION-AUDIT.md)。
...
- トーク履歴・メッセージ検索
- 予定・予約カレンダーを開く
- 写真・メディアライブラリ（店舗Botがいるルームだけ。下記参照）
...
**写真・メディアライブラリ**は、開いているM-talkルームに店舗Botがいる場合だけ、検索ランチャーから既存の `media.html` を開ける。

- 保存先はLINEと共通の非公開メディア台帳・ストレージ（`line_message_media` / `line-media`）。
- 開く際に一度だけ使えるワンタイムログイン認証を発行する。認証コードは発行後5分で失効し、
  交換済みの閲覧専用セッションは15分で失効する。
- 閲覧セッションは、発行元のM-talkルームに対応する店舗・ルームのメディアだけを取得できる。
  他の店舗・ルーム、LINE Summaryのメニュー、削除や設定変更の操作は利用できない。
- 通常のLINE Summaryから開くメディア閲覧は管理者用の全件画面であり、同じ台帳をより広い権限で表示する。
- M-talkからの画像投稿は、必ず投稿元のM-talkルームへ保存する。同じ店舗の別のLINEルームへは保存しない。
...
- 通知カードの「予約カレンダーを開く」もM-talk側の予約タブを同じ画面で開く。過去のLINE用リンク付きカードも対象。LINE側のGmail予約通知・本日の予約・「予約確認」もM-talkへ統一。LINEからはM-talkログイン後に本人の閲覧可能な店舗固定ルームと対象月を確認する。店舗所属・閲覧権限が必要で、リンクだけでは権限は付与されない。送信済みLINEリンクの旧使い捨てトークンは使用しない。
...
- 画像がメニューなら、Journal資料画面と同じ共通／メニュー／店舗専用プロンプトで商品名・価格・説明を抽出し、店舗Botが確認カードを返す。「この内容で資料へ登録」の時だけRAG資料を作り、「今回は登録しない」では元画像だけをM-talkに残す。価格が不足するメニューだけ1回再確認し、非メニューは追加AIへ送らない。

### ゴミ箱

- 管理権限のあるメンバーが、店舗固定以外のルームをゴミ箱へ移せる。ゴミ箱タブから復元できる。
- **完全削除は、管理権限を持つルーム作成者だけ**がゴミ箱から実行できる。ルーム名の再入力が必要。
- 本部管理者、または対象ルームと `manage_rooms` を付与された委任管理者の
  `chat-admin.html` からも、店舗固定以外のルームをゴミ箱へ移動・復元できる。
  復元不能な完全削除は本部管理者だけで、ルーム名の再入力と最終確認が必要。
...
| `can_review_access` | 新規登録・所属店舗変更を承認できる。本部だけが個人単位で付与 |
| `can_use_journal_ai` | 店舗限定の電子ジャーナルAIを利用できる。本部だけが個人単位で付与 |
...
3. **Botは通常のM-talk画面から削除できない**。Botの削除・復元は本部管理者、または
   対象店舗と `manage_bots` を付与された委任管理者の `chat-admin.html` だけで行い、
   過去履歴を消さない論理削除とする。
4. 通常のM-talk画面のルーム完全削除は、`can_manage` だけでは許可せず、**作成者本人**に限る。
   M-talk管理画面は別の管理者専用経路で、権限範囲内の店舗固定以外をゴミ箱経由で管理できる。
5. **ルーム管理と全体承認は別権限**。ルームを作った人、`can_manage=true`の人、M-talk委任管理者が、自動的に新規登録・所属店舗変更を承認することはない。
6. 「ジャーナルに聞く」は`can_use_journal_ai=true`に加え、M-talk利用可・対象ルーム参加・閲覧可・対象店舗への現在所属・ルーム内Botとの店舗一致を毎回確認する。権限取消、退出、所属取消、Bot差替え後は既存画面からも次のAPIで止まる。
...
Supabase Auth の利用可否には連動しない。本部管理者のほか、本部から範囲と操作を
明示的に付与された **M-talk委任管理者** が使える。

有効な全権管理者は、通常の`chat.html`で右上の自分のアイコンを押し、
「M-talk権限管理」から移動できる。この入口は`is_full_admin=true`の場合だけ表示し、
一般利用者、申請承認者、通常ルームの管理者には表示しない。リンクを表示できても、
`chat-admin.html`側では保存済み管理セッションまたは本部管理資格を改めて検証する。

### M-talk委任管理者

本部管理者は「委任管理者」で、LINE Report全体の管理権限を渡さずにM-talkだけを
管理するログインリンクを発行できる。リンクは24時間・1回限り。権限はAPI呼び出しごとに
DBから再確認されるため、委任を「即時停止」にすると既にログイン中の画面も次の操作から403になる。
管理範囲・操作権限・期限の変更、停止・再開のたびにセッション世代も変わるので、停止前のセッションは再開後も復活しない。
再開したときは本部が新しいログインリンクを発行する。
委任設定には有効期限が必須で、新規作成時は30日後・操作は閲覧だけから始める。

| 管理範囲 | 見える対象 |
| --- | --- |
| M-talk全体 | 全ユーザー・全ルーム。ただし許可された操作だけ |
| 選択店舗 | 選択店舗の所属ユーザー、店舗Bot、店舗ルーム。別店舗と無所属ルームは不可 |
| 選択ルーム | 指定ルームと参加者だけ。同じ店舗の未指定ルームも不可 |

操作は閲覧、監査閲覧、メンバー権限、ルーム管理、Bot管理、テンプレート適用、
全体ユーザー管理、監査復元を個別に付与する。全体ユーザー管理はM-talk全体の範囲だけ。
Bot管理はM-talk全体または選択店舗の範囲だけで、選択ルームの範囲には付与できない。
テンプレート適用にはメンバー権限、監査復元には監査閲覧と対象操作の変更権限も必要。
委任管理セッションから売上、予約、資料、ファイル、店舗設定など他の管理APIは利用できない。
新規登録・所属店舗変更の承認権限と電子ジャーナルAI権限は本部限定で、委任管理者は
`manage_users`を持っていても付与・変更できない。

### ユーザーの停止・削除・復元

1. 管理画面を開き、本部管理トークン、または本部から発行されたM-talk委任リンクでログインする。
2. 「ユーザー」で、M-talk 利用・1対1開始・ルーム作成・ユーザー一覧表示を個別に変更する。本部だけは全体承認・電子ジャーナルAIも明示付与できる。
...
- Bot行の「Bot削除」から、Bot名を再入力し、最終確認後に削除状態へする。
- `chat_users`を物理削除せず、`bot_deleted_at`を設定するため、過去メッセージと所属履歴は残る。
- 削除中はそのBot名義の新規投稿をDBで拒否し、予約通知・店舗Bot応答・検索応答に使用しない。
- 「削除済みBot」から「Bot復元」を押すと再び利用できる。
- 通常のM-talk画面にはBot削除・復元操作を出さない。

### 管理画面のルームゴミ箱

1. 「ルーム」で対象を選び「ゴミ箱へ」を押す。
2. 「ゴミ箱」タブから「復元」を選ぶ。本部管理者には「完全削除」も表示される。
3. 本部管理者が完全削除するときはルーム名を再入力し、最終確認する。

店舗固定ルームは対象外。管理画面の操作も監査ログへ記録し、完全削除は復元できない。
...
2. 「対象ルーム」で**このルームだけ**か、一覧でチェックした**複数ルーム**を選ぶ。
3. 「対象メンバー」で**対象ルームの全員**か**チェックしたメンバー**を選ぶ。
4. **「変更内容を確認」を押す**。誰がどう変わるかの差分が一覧表示される。この時点では**何も書き込まれない**。
5. 内容を確認してから「この内容で適用」を押す。
...
- **管理APIは本部フル管理、または専用の `mtalk_admin` 委任セッションだけ。** 従来の店舗・ルーム・cronスコープからは403。委任セッションは逆に、`/chat-admin/*`以外の売上・予約・資料・店舗設定APIを403にする。
- **委任はAPIとDBの二重検査。** APIで操作能力を確認し、service-role専用の`chat_admin_delegated_execute`が委任行をロックして、対象検査と実変更を同じDBトランザクションで行う。停止と操作の競合でも停止完了後の書込みを許さない。
- **復元不能なルーム完全削除は本部だけ。** 委任者の`manage_rooms`はゴミ箱と復元までに限定する。
...
7. 従来の店舗・ルーム限定セッションから `/chat-admin/*` を取得できない。
8. M-talk委任セッションから売上・予約・資料・ファイル・店舗設定APIを取得できない。
9. 店舗委任で他店舗、ルーム委任で未指定ルームを表示・変更できない。
10. 委任を停止または期限切れにすると、既存セッションも次のAPI呼び出しで403になる。
11. 停止と委任書込みの競合時も、停止完了後に変更が残らない。
12. 停止後に再開しても旧リンク・旧セッションは403になり、新しく発行したリンクだけ通る。
12. 委任管理者のルーム完全削除が403になる。
13. テンプレートの事前確認が1件も書き込まない。適用後に閲覧不可の行で他3権限が有効にならない。
14. 1対1へルーム管理者テンプレートを適用しても、招待・管理が無効のままになる。
15. 別の管理者が先に更新した監査ログの復元が409で止まる。同じ監査ログを2度復元できない。
...
| `chat_user_access` | M-talkだけの利用状態、通常全体権限、本部付与の承認・電子ジャーナルAI権限 |
...
| PATCH | `/chat-admin/users/:id` | 全体利用と通常全体権限、一時制限・理由。本部だけは承認・電子ジャーナルAI権限も変更 |
...
- **他人の発言は削除も編集もできない。** 自分のテキストは「…」→「編集」で直せる。
  直す前の文章は取り消し線で残る。直しても通知は送り直さない。画像・ファイル・カードは編集できない。
- **個人メモは送信されない。** ルームのタイムラインに挟まれるが、通信相手には一切届かない。
  他の参加者からは存在自体がわからない。
- **検索から途中の発言へジャンプしたあとも、新着は間を埋めてつなぐ。** 見ていた位置は維持する。最下部へ行きたいときは「最新へ」。
- **参加前の履歴は読めない。** 後から招待された人に過去ログは見えない。
- **発行済みの画像署名URLは即時失効しない**（最大1時間）。
- **新規登録は承認待ち。** 本部が`can_review_access`を明示付与した承認者だけが許可でき、許可後も閲覧のみで始まる。通常ルームの管理者や一般メンバーには承認カードを見せない。
