...
4. 許可／不許可カードは専用の「管理者通知」ルームにだけ届く。予約通知や店舗ルームの一般メンバーには見えない。許可されると申請した所属店舗が付き、閲覧のみで始まる。
...
- **新規登録は承認待ち。** 管理権限を持つ人が許可するまで使えず、許可後も閲覧のみで始まる。承認カードは「管理者通知」ルームにだけ届き、予約通知や店舗ルームの一般メンバーには見えない。
