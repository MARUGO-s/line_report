...
- 本部フル管理セッションの `chat-admin.html` からも、店舗固定以外のルームを
  「ゴミ箱へ移動 → 復元 / 完全削除」の順で管理できる。管理画面の完全削除も
  ルーム名の再入力と最終確認が必要。
...
3. **Botは通常のM-talk画面から削除できない**。Botの削除・復元は本部フル管理セッションの
   `chat-admin.html`だけで行い、過去履歴を消さない論理削除とする。
4. 通常のM-talk画面のルーム完全削除は、`can_manage` だけでは許可せず、**作成者本人**に限る。
   本部管理画面は別の管理者専用経路で、店舗固定以外をゴミ箱経由で管理できる。
...
2. 「ゴミ箱」タブから「復元」または「完全削除」を選ぶ。
3. 完全削除ではルーム名を再入力し、最終確認する。
