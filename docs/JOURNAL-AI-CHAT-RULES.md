...
この例外はJournal Reportのサーバー確定計算に限定します。フードコートの通常分析・専門AIメモには、前提や仮定ラベルを付けても数値を独自に作る権限はありません。提供された実績・現場設定値・コード側の事前計算値を出所付きで引用し、不足時は必要な計測を案内します。
...
4. 売上の期待値（1日・月間）
5. KPI目標値（セット率・購入会計の客単価上昇額・テイクアウト比率・廃棄率）
6. 撤退・縮小ラインの数値
7. イベントのない通常営業日の見込み
