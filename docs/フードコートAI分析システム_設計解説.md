...
> 2026-09-21: Q&Aは期間→手法→必要な売価/原価確認。選んだ手法の知識パックだけを分析前に読む（[management-warehouse](./management-warehouse/README.md)）。KGI/KPI/KFIは改善管理時のみ。共通規則は `_shared/business_goal_metrics.ts`。日次/期間キャッシュは `foodcourt-analysis-ai-v26-method-packs`。
...
現行キャッシュ: `foodcourt-analysis-ai-v26-method-packs`（Q&A・日次・期間）。旧表記 `v16-loop-learning` は履歴。
