...
### M-talkメニュー画像の確認登録（2026-09-02）

店舗にひも付くM-talkルームへ画像を送ると、既存の画像AI判定がメニューを検出し、同じ共通資料・
共通メニュー・店舗専用プロンプトと、同じ正規化・品質検査を使って結果カードを返信する。
画像を送っただけでは資料へ保存せず、カードの「この内容で資料へ登録」を押した時点で初めて
`store_knowledge_documents`、非公開`store-knowledge`、`store_knowledge_chunks`へ登録する。
「今回は登録しない」では資料を作らず、元画像はM-talkの非公開メディアへ残す。

- 1回目の画像判定に統合し、非メニュー画像を追加AIへ再送しない。メニューで商品・価格が不足する場合だけ1回再確認する
- 登録前の解析結果はservice-role専用`chat_menu_knowledge_drafts`へ7日だけ保持する
- 決定APIはM-talk JWT、送信権限、ルーム店舗対応、下書き状態をサーバーで再検査する
- 元画像は同一ルームの`chat-images/groups/<group_id>/...`だけを受理する
- 登録時はSHA-256で同一店舗の有効資料との重複を確認し、既存なら複製せず同じ資料IDへ解決する
- 登録後はJournal Reportの「資料」タブが閲覧・編集・無効化の正本になる
