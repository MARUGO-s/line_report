...
- 分析モードは自由質問（Q&A）、日次サマリー、期間サマリー、週次報告です。Q&Aは期間のあと、質問に合う分析手法を候補表示します。複数選択でき、選んだ角度だけを実行します。重ね聞きは前回の手法を引き継ぎます。「2026年6月」「6月と8月を比較」「今月」「表示中の日」「保存済み全期間」を指定でき、「期間を変更」や「会話をクリア」で戻せます。売価・原価が要る分析の前に、入力欄か「全部お任せ」を確認します。
