...
- ユーザー全体の利用停止・期限付き制限・論理削除と、ルームごとの閲覧・送信・招待・管理権限を設定できます。新規登録はルーム管理権限を持つ人の許可が必要で、許可後は閲覧のみで始まります。許可カードは管理者本人のトークにだけ届き、店舗ルームの一般メンバーには見えません。
...
- 補助・運用・レガシーコード: 37件
- admin-api静的ルート: 136件
- SQL migrations: 277件（全件の構文・関係はGraphify/knowledge:checkで監査）
- テストファイル: 71件
