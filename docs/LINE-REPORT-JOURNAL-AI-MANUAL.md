...
- KGI（成果）→KPI（客数・客単価など）→KFI（店頭案内・提案など現場行動）の対応を分析と回答に含めます。KFIの担当候補・タイミング・記録方法を示し、実行後は逆方向に成果を検証します。採算は別枠で確認し、未計測値や未設定の目標は作りません。
...
- 改善相談ではKGI（成果）→KPI（成果の分解）→KFI（現場行動）を関連づけて回答します。KFIを財務指標とはせず、利益・原価などは別途確認します。3指標の名前だけでは数値試算を起動しません。
...
- 全分析の結果にKGI（成果）→KPI（客数・客単価・セット率など）→KFI（店頭案内・セット提案など現場行動）の関係を含めます。KFIの担当候補・タイミング・記録方法を示し、採算は別枠で確認します。共有席などの未計測値や未設定目標は作らず、一般的な目安を店舗目標へ転用しません。日次・期間・週次は既存見出し内へ記載します。
...
- 通常Q&Aでは数値を創作しません。新商品KPI・価格・損益分岐などの試算を明示依頼した場合だけ、コードが保守／標準／強気の3シナリオを計算します。試算前提は画面入力→店舗営業情報の保存済み値→仮置きの順で使い、【仮定(入力)】【仮定(シナリオ)】を付けて実績と分けます。入力欄は今回の会話専用で店舗情報を書き換えません。質問文に書いただけの数値は入力前提として採用しないため、試算前提欄へ入力してください。
