...
- 分析は目的に合う方法を選び、改善策が必要なときだけKGI（成果）→KPI→KFI（現場行動）へ落とします。KFIの担当・タイミング・記録方法を示し、実行後は逆方向に成果を検証します。採算は別枠。未計測値や未設定の目標は作りません。
...
- 改善相談では実績の分解を先に見て、必要なときだけKGI（成果）→KPI（成果の分解）→KFI（現場行動）を関連づけます。KFIを財務指標とはせず、利益・原価などは別途確認します。3指標の名前だけでは数値試算を起動しません。
...
- 目的に合う角度だけを使い、毎回全部は並べません。売上ABCは原価不要、人気×採算だけ原価が必要です。改善策を書くときだけKGI→KPI→KFIを含めます。KFIの担当・タイミング・記録方法を示し、採算は別枠。未計測値や未設定目標は作らず、一般的な目安を店舗目標へ転用しません。日次・期間・週次は既存見出し内へ記載します。
...
- 分析モードは自由質問（Q&A）、日次サマリー、期間サマリー、週次報告です。Q&Aは期間のあと、質問に合う分析手法を候補表示します。複数選択でき、選んだ角度だけを実行します。重ね聞きは前回の手法を引き継ぎます。「2026年6月」「6月と8月を比較」「今月」「表示中の日」「保存済み全期間」を指定でき、「期間を変更」や「会話をクリア」で戻せます。売価・原価が要る分析の前に、入力欄か「全部お任せ」を確認し、任意で最終目標KGIも受けます。選んだ手法の短い知識パックだけを分析前に読みます。
...
- 商品販売数・購入会計比率・会計時間帯・同時購入は観察に先に使い、改善や明示試算の段階でKPIへ対応づけます。会計数は客数、同時購入はセット率ではありません。新商品の時間別目標は明示試算時だけ、観測した会計時間構成で3シナリオ配分し【仮定(シナリオ)】とします。原価・廃棄・KFI実行件数など未記録の数値は創作しません。
...
- 入力欄の数値は試算を依頼しなくても【仮定(入力)】として分析へ渡します。粗利・目標の手法を選ぶと、原価未登録でも推測値の目標を出します。分析のみでは新しい目標数値を生成しません。回答末尾の「今回、分析へ渡した前提」で値と出所、試算実行の有無を確認できます。過去の回答は自動で書き換わらないため、修正後の画面で再質問してください。
...
- 新方式への切替は月曜日、朝の時間枠（JST09:00より前）の1日前予測で共通21日以上、客数と売上のMAEがともに5%を超えて改善し、前後半も改善する場合だけです。同曜日基準以外の方式は同曜日基準にも勝つ必要があります。採用後28日は再切替を待ちます。
...
| `supabase/functions/_shared/business_goal_metrics.ts` | FCT-02 / JAI-01 / JAI-02 / DEV-02 |
...
| `supabase/functions/_shared/foodcourt_management_packs.ts` | FCT-01 / FCT-02 / FCT-03 / FCT-04 / FCT-05 / FCT-06 / DEV-02 |
| `supabase/functions/_shared/foodcourt_qa_methods.ts` | FCT-01 / FCT-02 / FCT-03 / FCT-04 / FCT-05 / FCT-06 / DEV-02 |
