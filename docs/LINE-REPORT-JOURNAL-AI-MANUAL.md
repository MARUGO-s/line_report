...
- 店舗用ログインは自店舗と発行目的（予約閲覧、小口現金閲覧、売上分析、フードコート等）へ固定されます。別の管理画面や許可されていない書込みを開いてもAPIが拒否します。全店切替や横断集計は本部管理者向けです。
...
**検索語:** 店舗情報 / 定休日 / 定休 / ランチ / ディナー / 特別営業 / カレンダー / イベント / 過去売上同期 / ワイン / ml / グラス / デキャンタ / ボトル / ペアリング
...
- マルゴエスでは、フードコート分析ページの全データではなく、期間内の東京ドームイベントと直近のコート内順位・シェア・タイプ判定だけを会場背景として足します。コート日報の売上は背景で、店舗売上の正本はジャーナルです。
...
**検索語:** 標準ai分析 / ai分析 / コンサル分析 / 経営分析 / ワイン戦略 / ダッシュボード / 履歴 / 根拠 / 外部知見 / 外部検索 / grok / perplexity / 数値照会 / 毎回検索 / 戦略質問 / luna / claude / 東京ドーム / フードコート / marugo s
...
**検索語:** aiチャット / ジャーナルに聞く / 質問 / 期間比較 / 月次推移 / 客単価 / 曜日 / 時間帯 / 改善案 / pdfにする / clarifier / 確認 / フードコート深掘り / ブースト / api料金 / 専門ai / 統合分析
...
- Journal ReportのマルゴエスAIは、通常は期間内イベントと直近コート内順位の軽量要約だけを会場背景として参照します。Web版AIチャットで必要な質問に限り、利用者が時間とAPI料金の注意を確認して同意した後、フードコート専門AIを追加しJournal分析と1本へ統合できます。
...
**要点:** M-talk専用の委任範囲を含む、利用者・ルーム・Bot・監査機能を説明する。
...
- M-talkの全権管理者には、通常のトーク画面で右上の自分のアイコンを押すと「M-talk権限管理」が表示されます。一般利用者、申請承認者、通常のルーム管理者には表示されません。遷移先では保存済みの管理セッションまたは本部管理資格を改めて検証します。
...
- ユーザー全体の利用停止・期限付き制限・論理削除と、ルームごとの閲覧・送信・招待・管理権限を設定できます。ルームを作成・管理できても、新規登録や所属店舗変更を承認する全体権限にはなりません。この承認権限は本部管理者が利用者ごとに明示付与し、許可カードはその承認者だけが入る管理者通知ルームに届きます。許可後の一般利用者は閲覧のみで始まります。
- 「新規登録・所属店舗の変更を承認できる」と「電子ジャーナルAIを利用できる」は本部限定の付与権限です。委任管理者や一般のルーム管理者は、自分自身を含めて付与・変更できません。変更は監査ログへ残り、承認権限の取消時は管理者通知ルームからも同じ処理内で外れます。
- 店舗Bot経由の予定・設定・メディア・電子ジャーナルAIは、ルーム参加だけでなく対象店舗への現在承認済み所属も毎回確認します。所属取消や店舗Botの退出・差替え後は、保存済みの画面やセッションも次のAPI操作から利用できません。
...
**検索語:** m-talk管理 / chat-admin / 利用停止 / 論理削除 / ルーム権限 / 権限テンプレート / 一括設定 / ユーザー別アクセス / 監査ログ / 復元 / bot削除 / bot復元 / botを削除 / botは削除 / 通常画面 / 普通のトーク画面 / トーク画面から削除 / ルームゴミ箱 / 委任管理者 / m-talk専用管理者 / 店舗限定管理 / ルーム限定管理 / 監査専用 / 右上アイコン / アカウントメニュー / 権限管理リンク

**主な実装根拠:** `public/chat.html` / `public/chat/permissions.js` / `public/chat/rooms.js` / `public/chat-admin.html` / `supabase/functions/admin-api/index.ts` / `supabase/functions/_shared/chat_admin_delegation.ts` / `supabase/migrations/20260910010000_chat_delegated_admin.sql` / `docs/CHAT-ADMIN-PERMISSIONS.md` / `tests/chat_admin_permissions.test.mjs` / `tests/chat_admin_templates.test.mjs` / `tests/chat_admin_delegation.test.mjs` / `tests/chat_admin_delegation_scope.test.ts`
...
- chat-knowledgeはM-talk店舗ルームのBot処理と1対1の使い方AIを担当し、店舗Bot処理の前に送信者の現在承認済み店舗所属を確認します。chat-searchは検索、chat-pushはWeb Pushを担当します。
...
- 店舗用ログインは自店舗・用途・HTTP操作へ固定され、画面上でボタンを隠すだけでなくAPIでも他店舗・別機能・管理者操作を拒否します。M-talkの閲覧・送信・管理は全体利用状態とルーム権限で判定し、店舗Bot経由の操作は現在承認済みの店舗所属も必須にします。
- 画像・原本・添付は非公開保存し、参加ルーム等を再確認したうえで必要時だけ一時URLで表示します。内部定期処理の鍵は公開ブラウザ鍵と分離し、対象処理以外の管理APIには利用できません。
...
| `supabase/functions/_shared/chat_admin_delegation.ts` | ADM-01 / ADM-03 / SEC-01 / DEV-02 |
...
| `supabase/functions/_shared/internal_cron_auth.ts` | SEC-01 / DEV-02 / DEV-03 |
...
| `/auth/chat-journal-link-login` | SYS-02 / SEC-01 / DEV-02 |
...
| `/chat-admin/` | ADM-01 / ADM-03 / OPS-01 / DEV-02 |
| `/chat-admin/delegations` | ADM-01 / ADM-03 / OPS-01 / DEV-02 |
...
| `/foodcourt/journal-brief` | FCT-01 / FCT-02 / FCT-03 / FCT-04 / DEV-02 |
| `/foodcourt/journal-deep-analysis` | FCT-01 / FCT-02 / FCT-03 / FCT-04 / DEV-02 |
