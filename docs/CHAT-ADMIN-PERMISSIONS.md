...
通常ルームの`can_manage`はそのルームだけの権限であり、新規登録・所属店舗変更の全体承認、
本部限定権限の付与、他ルームや他店舗の管理には一切使わない。
...
| `can_review_access` | 新規登録・所属店舗変更を全店舗で承認できる。本部だけが明示付与 |
| `can_use_journal_ai` | 1対1から店舗限定の電子ジャーナルAIを利用できる。本部だけが明示付与 |
...
「管理者通知」ルームへ参加できるのは`can_review_access=true`の有効な人間利用者だけ。
通常ルームや店舗ルームの`can_manage`では参加・閲覧できず、一般メンバーの未読にも載せない。
承認権限を外す操作は、同じDBトランザクション内でこのルームからも外す。

電子ジャーナルAIは、`can_use_journal_ai=true`、M-talk全体利用可、対象1対1への参加、
`can_view=true`、対象店舗への現在承認済み所属をすべて満たす場合だけリンクを発行する。
リンク交換は専用endpointでのみ行い、交換後セッションは30分、店舗・ルーム・本人へ固定する。
各API呼び出しで所属とルーム内店舗Botの一致まで再検査し、
予約者情報、資料・店舗設定、原本取込、削除・更新等の管理経路には広げない。
...
| PATCH | `/chat-admin/users/:id` | 全体利用、通常全体権限、一時制限・理由。本部だけは承認・電子ジャーナルAI権限も変更 |
...
- `chat_users`の本人更新では`id / created_at / is_bot / store_key / bot_deleted_at / bot_deleted_by`をDBトリガで元値へ戻し、一般利用者がBotや店舗Botへ自己昇格できないようにする。
- `can_review_access`と`can_use_journal_ai`の更新は本部管理セッション＋service-role専用RPCだけ。委任管理者がpayloadへ項目を追加してもAPIで403、authenticatedからRPCを直接呼んでも権限なしになる。
...
11. 通常ルーム作成者／`can_manage=true`の利用者が新規登録・所属店舗変更を承認できない。
12. `can_review_access=true`の本部指定者だけが承認でき、取消直後に管理者通知ルームを読めない。
13. `can_use_journal_ai=false`ではボタン・リンク発行・既存セッションの次回APIがすべて拒否される。
14. 電子ジャーナルAIセッションから予約、資料、店舗設定、取込、削除APIを取得できない。
15. 店舗所属を取り消すと、過去のカスタムルームに残っていても店舗Bot処理、予定、設定、メディア、Journal AIの既存セッションが拒否される。
16. 委任レコードを停止すると、既存セッションが次のAPI呼び出しで403になる。
17. 停止更新と委任書込みを同時実行し、DB行ロックにより停止完了後の書込みが0件になる。
18. 停止後に再開しても旧`session_version`のリンク・セッションは403になり、新しいリンクだけ通る。
19. 委任管理者の完全削除が403で、本部だけが実行できる。
20. テンプレートの`dry_run`が1件も書き込まない。適用後に`can_view=false`の行で他3権限がtrueにならない。
21. 1対1へ`room_admin`テンプレートを適用しても、招待・管理がfalseのままになる。
22. 別の管理者が先に更新した監査ログの復元が409で止まる。同じ監査ログを2度復元できない。
