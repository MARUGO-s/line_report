...
本番反映済み。最新SHA `a0795a92`。日次/期間キャッシュは `foodcourt-analysis-ai-v31-capacity-table`。保存済み回答・旧PDFは自動では変わらない。画面更新後に再質問する。

- 新商品のKPI付録テーブルの「予想販売数/日」で、焼成上限×廃棄控除に頭打ちしたシナリオは「◯個（観測◯個／焼成上限◯個）」とセル内に観測値（丸め前の類似商品アンカー）と上限を併記する。従来は表の上に1行だけ「観測◯個→上限◯個」と書いており、保守/標準/強気の個数差が小さく見えても、それが需要差ではなく生産能力の頭打ちだと表単体では分かりにくかった。表示ロジックと`FOODCOURT_KPI_POLICY`の指示はいずれも商品名に依存しない汎用実装（`row.outlookExceedsCapacity`/`row.observedUnits`/`row.capacityUnits`）で、クロワッサン以外の今後の商品分析にも自動的に適用される。
