...
本番反映済み。最新SHA (反映後に追記)。日次/期間キャッシュは `foodcourt-analysis-ai-v32-generic-kpi-labels`。保存済み回答・旧PDFは自動では変わらない。画面更新後に再質問する。

- 【重要・不具合修正】無関係な重ね聞きへ前回選んだ「目標・損益分岐・撤退」「粗利・採算」を無条件に引き継いでいたため、クロワッサン導入のKPI試算を1回行った後、全く話題の違う質問（例:「客数不足を解消する施策は？」）を同じ会話で聞いても、その質問文自体はKPI試算を一切求めていないにもかかわらず、前回のクロワッサンKPI試算（前回入力した売価・焼成前提を使った新商品トライアルの表）がそのまま付録に付いてしまい、しかも本題の回答が空（「回答を生成できませんでした」）になる実例が見つかった。`public/foodcourt-qa-planner.js`の`nextTurn()`で、重ね聞き時に前回の`state.methods`を無条件に引き継ぐ`skipMethods`ロジックを修正: 今回の質問自体が`wantsKpiTargets()`でKPI試算を求めていない場合は、引き継ぐ手法から`kpi`・`margin`だけを外す（他の軽い分析角度はそのまま引き継ぐ）。今回の質問が改めてKPIを求めていれば従来通り追加される。回帰テスト追加（`foodcourt_qa_planner.test.mjs`）。この修正は「話題が変わったのに前回の重い数値試算だけが居座る」ケースを防ぐもので、本題の回答が空になる件は、巨大化した無関係プロンプトが一因の可能性はあるが未確証。再発時は新しいログで別途調査する。
- KPI試算前提の項目名をクロワッサン（焼成）専用の言葉からどの商品にも使える言葉へ変更した。「1回に焼ける個数」→「1回の仕込みで作れる数量」、「1日の焼成回数」→「1日の仕込み回数」、「仕込み・焼成に割ける人員」→「仕込み・提供に割ける人員」、「1日の仕込み・焼成時間」→「1日の仕込み・提供時間」。表・AI回答中の「焼成上限」も「仕込み上限」に統一。`public/foodcourt.html`のQ&A入力欄、`public/jnm/jnl2txt.html`の店舗設定タブ、`kpi_scenario.ts`の`KPI_ASSUMPTION_LABELS`の3箇所を揃えて変更。内部のフィールド名（`bakeBatchUnits`等）・保存済みDB値は変更していないため、既存の店舗設定は引き続き有効。
- 新商品のKPI付録テーブルの「予想販売数/日」で、仕込み上限×廃棄控除に頭打ちしたシナリオは「◯個（観測◯個／仕込み上限◯個）」とセル内に観測値（丸め前の類似商品アンカー）と上限を併記する。従来は表の上に1行だけ「観測◯個→上限◯個」と書いており、保守/標準/強気の個数差が小さく見えても、それが需要差ではなく生産能力の頭打ちだと表単体では分かりにくかった。表示ロジックと`FOODCOURT_KPI_POLICY`の指示はいずれも商品名に依存しない汎用実装（`row.outlookExceedsCapacity`/`row.observedUnits`/`row.capacityUnits`）で、クロワッサン以外の今後の商品分析にも自動的に適用される。
- 【重要・AI構成の変更】新商品の想定販売数（焼成上限で頭打ち済み）を、店内で最も個数の出ている既存商品の実績（当店の実測値、業界目安ではない）と比較する「前提の妥当性チェック」を追加した。想定が既存の最多販売商品の実績以上なら過大評価の可能性として付録テーブル・AI回答の両方に明記する。反証AI④（品質管理担当）は従来「新しい施策のKPI試算は後段のサーバー確定ブロックが担当する」として試算の中身を一切見ていなかったが、この比較結果だけは受け取り、過大/過小評価の可能性を指摘する担当を新たに持つ。算術（掛け算・割り算）自体は引き続きサーバー確定計算（`kpi_scenario.ts`）の担当で、反証AIは前提の目利きだけを行う。詳細は `docs/フードコートAI分析システム_設計解説.md`（反証AI④の節）。
- 新商品のKPI見込み（予想売上/日・寄与率・上積み）は「単品のみ」と「セット込み（ドリンク・ワインの上乗せを加重平均）」の両方を付録テーブルに併記する。セット選択比率・上乗せ額（ドリンク+◯円／ワイン+◯円）は表の上の注釈で開示し、実測のセット購入率ではないことを明記する。純増ギャップも両基準で出す。v28では単品のみで計算しながら「セット込み」と説明しており、実際の値と説明が食い違っていた。
- 新商品のKGIは店舗の最終成果（現状の店舗日次売上＋純増目標）。施策のセット込み売上そのものはKPIであり店舗KGIではない。単品売上とだけ比較していたv27以前の混同を修正済み。
- 新商品のKPI見込み（予想販売数/日・寄与率・上積み）は焼成上限×廃棄控除（`sellableCapacityUnits`）で頭打ち。類似商品アンカーが他商品合算で上限を超えたら「観測◯個→上限◯個」と付録テーブルに開示する。v26では未クランプで、既存トップ商品（カレー・ビール等）の合算日販がそのまま新商品の見込みに使われ、焼成能力を大幅に超過していた。

- 流れ: 質問 → 期間 → 分析手法 →（KPI/粗利なら）売価・原価の確認 → 分析。重ね聞きは前回回答が本文。
- 売価・原価が要る分析の前に、入力欄・チャット数値・「全部お任せ」を確認する。任意で最終目標KGI。客単価や類似SKUの単価を新商品の店頭価格にしない。
- 選んだ手法の短い知識パックだけを分析前に読む。倉庫は `docs/management-warehouse/`。ベクトルRAGではない。
- KPIはKGI→ギャップ→CSF1つ。売上だけで評価しない。人時売上は労働時間があるときだけ。業界目安を当店目標にしない。
- Q&AのPDFは金茶色の分析例レイアウト。「この回答を出力」と「会話をPDF」。シナリオ表は注釈を表の上に1行、セルへ【仮定(シナリオ)】を複製しない。
- 新しい施策は実績なし前提で、今の店舗日次売上と類似商品から寄与・上積みをコード計算する。試算が動いたら末尾に表を必ず付ける。
- 正本: `docs/BUSINESS-GOAL-METRICS.md`、`docs/FOODCOURT-JOURNAL-LINK.md`、`docs/management-warehouse/README.md`。

以下の「未配備」は当時の記録。上記現行が配備状態の正本。

## 2026-09-21 新商品販売分析はジャーナル推移から見込みを出す

- 販売分析・販売戦略・見込みはKPI試算ゲートに入れる。ゼロからの創作ではなく、照合済み商品の日次個数と月次売上比で3シナリオをコード計算。
- 会話文脈に「導入」があれば「販売分析をお願い」でも起動。重ね聞きでも確定ブロックの目標・撤退を省略しない。配備済み（現行節）。

## 2026-09-21 フードコートQ&Aは前回回答への重ね聞き

- 直前のassistant回答がある同じ会話は follow-up。前回回答が本文、今回質問はその内容への質問、今回集計は裏付け。全体書き直しと「次の一手」必須化を外す。
- 「最初から」「やり直して」「新しく分析」は通常の全体分析。数値創作禁止は維持。回帰は foodcourt_kpi の journal follow-up と判定テスト。配備済み（現行節）。

## 2026-09-20 フードコートQ&Aの追加質問が対象を失う

- ブラウザは直前8件を送っていたが、商品抜粋と専門AIは今回の質問文だけを見ていた。全期間の上位商品に埋もれると、追加質問で「それ」の商品明細が消える。
- 会話テキストを商品名マッチと併売に使い、専門AI/反証にも直前会話を渡す。金額は今回の集計、対象は引き継ぐ。回帰は journal_link / foodcourt_kpi / qa_route。
- 本番反映済み: `0778268`、Edge `35516945177`、`admin-api v1215`。未認証POSTは401。保存済み回答は更新しない。
...
- 本番反映済み: `972f516`、Pages `35514156846`、Edge `35514156878`。`admin-api v1212` / `ai-analyze v292`。未認証POSTは401。保存済み回答は更新しない。
...
## 2026-09-21 新しい施策の上積み推測

- 新しい施策・新商品に施策実績は無い。`buildFoodCourtInitiativeUplift` が今の店舗日次売上【実績】を分母に、類似商品見込みから寄与率と上積みを出す。上積み率は保守0.3／標準0.55／強気0.85の仮定。
- `isKpiScenarioRequest` に施策の貢献・上積み・分析を追加。配備済み（現行節）。

## 2026-09-21 フードコートQ&Aの分析手法選択

- 期間確定のあと、質問内容から候補にした分析手法をチップで選ぶ。選んだ角度だけを専門AI/統合AIの本題にする。重ね聞きは前回の手法を引き継ぎ、「最初から」で選び直す。
- 粗利・目標を選ぶと `prepareFoodCourtKpiScenario({ force: true })`。原価未登録でも【仮定(シナリオ)】の推測値で目標を出す。実績だけ/試算不要なら起動しない。
- 回帰は `foodcourt_qa_planner.test.mjs` / `foodcourt_qa_route.test.mjs` / `foodcourt_kpi.test.ts`。配備済み（現行節）。
