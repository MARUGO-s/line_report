...
- 会話文脈に「導入」があれば「販売分析をお願い」でも起動。重ね聞きでも確定ブロックの目標・撤退を省略しない。未配備。

## 2026-09-21 フードコートQ&Aは前回回答への重ね聞き

- 直前のassistant回答がある同じ会話は follow-up。前回回答が本文、今回質問はその内容への質問、今回集計は裏付け。全体書き直しと「次の一手」必須化を外す。
- 「最初から」「やり直して」「新しく分析」は通常の全体分析。数値創作禁止は維持。回帰は foodcourt_kpi の journal follow-up と判定テスト。未配備。
...
- `isKpiScenarioRequest` に施策の貢献・上積み・分析を追加。未配備。
...
- 回帰は `foodcourt_qa_planner.test.mjs` / `foodcourt_qa_route.test.mjs` / `foodcourt_kpi.test.ts`。未配備。
