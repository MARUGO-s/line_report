...
- **本番 dry_run の通し実行は、この環境に管理トークンが無いため未完了。**
  2026-08-27に確認した範囲:
  - Pages `chat-admin.html` / `chat.html` HTTP 200、管理ログイン画面表示。
  - 未認証の `/chat-admin/state`・`/templates`・`POST /templates/apply`・
    `POST /audit/1/revert` はいずれも 401 `Unauthorized.`。
  - 本番 `chat.html` はログイン済みセッションでトーク一覧・Botタブまで到達。
  - 管理画面でルームを選び「変更内容を確認」（`dry_run:true`）は、
    本部トークンを持つ人が本番で1回押す必要がある。行も監査も書かない。

### 次に勧める実装

**複数ルームへの一括適用（UI）** — **実装済み（2026-08-27）**。
ルーム一覧のチェックと「対象ルーム：チェックしたルーム」で
`group_ids` 配列をプレビューへ渡す。DB変更なし。
