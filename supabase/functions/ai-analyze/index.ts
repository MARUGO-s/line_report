...
3. ワイン提供量(ml): salesData.wineVolumeAnalysis または店舗営業情報の換算設定×Glass／Bottle／ペアリング点数で合計mlを必ず示す（換算率捏造禁止）
4. フード×ワインのペアリング・クロスセルによる客単価向上（昼夜別）
5. 同エリアの姉妹店連携は「その店舗のエリアで妥当な場合のみ」（新宿三丁目密集の話を他エリアに転用しない）
6. 提供データに無い原価・在庫・利益は捏造しない
7. salesData.topProducts に「コース系（合算）」がある場合は、コース／おまかせを合算した順位を正とし、「★ドリンク」等のPOS大分類を単独最大商品と断定しない。カテゴリ合計（foodTotal/drinkTotal）と商品ランキングを混同しない`;
...
- ワイン提供量(ml)の要点（Glass／Bottle／ペアリングの合計mlまたは主要内訳）を必ず一言含める
...
- **ワイン提供量(ml)・必須**: salesData.wineVolumeAnalysis（Glass／Bottle／ペアリングの点数・ml・合計ml・換算設定）を独立した小見出しで必ず示す。換算率や点数は与えられた数値のみ。銘柄名のみのボトルは対象外と明示してよい
...
- **収益性の弱点**: 昼夜それぞれのドリンク／ワイン注文率、低単価偏り、グラス止まりでボトルへ繋がっていない可能性（ml内訳を根拠に使える場合は使う）
- **リスク分析**: 特定曜日や一部主要商品への依存

## 4. 売上・利益最大化のための具体的営業アドバイス＆アクションプラン 💡
- **【即効・ワイン営業アドバイス（明日からできる施策）】**
  - 客単価＋300〜500円を狙うグラスワイン追加提案、ペアリング一言提案、ボトルアップセル（昼夜別の優先策）。ml／点数の事実に紐づける
...
- 抽象的な表現を避け、提供されたデータ内の具体数値（売上金額、人数、単価、構成比％、点数、ワイン提供ml）を豊富に引用し、数値的根拠を持って論述してください。
...
- ワイン提供量(ml)は標準分析の必須節です。salesData.wineVolumeAnalysis または店舗営業情報の換算節があるときは必ず引用し、換算率を捏造しないでください。`;
