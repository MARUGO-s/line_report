  ...
  for (const month of monthList) {
    try {
      ...
      const { data: existingData, error: existingError } = await supabase
        .from("saved_reports")
        .select(
          "id, title, created_at, updated_at, sourceMonths:data->sourceMonths, autoGeneratedFromPosJournal:data->autoGeneratedFromPosJournal",
        )
        .eq("store_partition_key", storeKey)
        .is("deleted_at", null)
        .order("created_at", { ascending: false })
        .limit(5000)
      if (existingError) {
        throw new Error(`既存レポート確認に失敗しました: ${existingError.message}`)
      }
      const existingRows: Record<string, unknown>[] = []
      for (const row of Array.isArray(existingData) ? existingData : []) {
        if (isRecord(row)) existingRows.push(row as Record<string, unknown>)
      }
      const now = new Date().toISOString()
      const actions: Array<{
      ...
      const payloads = actions.map((entry) => {
        ...
        const reportData = { ...report.data, id, title: report.title }
