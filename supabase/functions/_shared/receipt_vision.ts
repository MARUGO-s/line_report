...
function buildSeiyuExpenseDetailPrompt(): string {
  return [
    ...
    '必ず JSON の receipt.line_items を商品行単位で返す。「合計42点」は購入個数であり明細42行ではない。商品行先頭の売場/商品コードの数字（例「49」「94」）と軽減税率印「※」は name に含めず、税率判定だけに使う。例: 「※94 パプリカ ¥99 ×40 ¥3,960」は name="パプリカ ×40", price="¥3,960", rate=8 の1行。「49 シンショクヒンヨウ ¥6 ×2 = ¥12」の袋代行は、品名が読みにくくても name="レジ袋 ×2", price="¥12", rate=10 の1行として絶対に落とさない。',
