...
export async function replyLineMessages(
  ...
  if (!response.ok) {
    ...
    return { ok: false, error: `LINE reply API ${response.status}: ${errText}` }
  }
  return { ok: true }
}
...
export async function replyLineText(
  ...
  return replyLineMessages(replyToken, [{ type: 'text', text: String(text ?? '').slice(0, 4900) }], channelAccessToken)
}
...
export async function replyLineFlex(
  ...
  return replyLineMessages(replyToken, [flexMessage], channelAccessToken)
}
