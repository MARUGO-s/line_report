...
分析期間・データ源ごとの開始日/終了日・日数・不足範囲を説明する。月別の記録日数は営業日数とは限らず、未取得日は0円としない。nullは未確認。税込と税抜を区別し、数値の創作や再計算はしない。金額・件数は今回取得した実績を優先する。追加質問では直前の対象商品・店舗・期間を引き継ぎ、新しい別テーマとして始めない。`;
