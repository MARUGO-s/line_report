...
// 2026-09-20: 前提を明示したシナリオ試算の許可＋通常営業日ベースラインの必須化で v20。旧キャッシュを再生成させる。
...
const FOODCOURT_ACTION_FORMAT_RULE =
  ...
  '価格・客数・増加率などの数値は「実績値」「現場が決めた目標」「検証用の判定ライン(設定根拠つき)」' +
  '「前提を明示したシナリオ試算」のいずれかに限る。根拠のない断定的な予測は避けるが、' +
  '前提（売価・原価・提供能力・想定客数）を明示できる場合は、保守/標準/強気のシナリオ試算として数値を出してよい。' +
  'その数値には必ず「仮定」と前提の出所を添え、実績値と同じ表・同じ合計に混ぜない。前提すら立てられない項目だけ「未計測」とすること。'
...
export async function answerFoodCourtQuestion(
  ...
  const system = [
    ...
    `(5-2) 施策効果の実績値が無い場合、根拠のない断定的な予測はしない。ただし「出せません」で止めない: 売価・原価・提供能力・想定客数などの前提を明示できるなら、保守/標準/強気の3シナリオ試算として増収額・客数リフト率・客単価上昇額を数値で示してよい。その数値は「仮定」と前提の出所を必ず添え、実績値と同じ表・同じ合計に混ぜない。前提すら立てられない項目だけ「検証用の目標/判定ライン」または「未計測」とする。施策には対象客・実施方法・観測KPIを必ず付ける。`,
