...
export const MTALK_HELP_SECTIONS: MtalkHelpSection[] = [
  ...
  },
  {
    ...
    keywords: ['ログイン', '新規登録', '登録', 'アカウント', 'メール', 'パスワード', 'プロフィール', '表示名', '名前', 'アイコン', '初回', '承認', '許可', '所属', '店舗'],
    content: [
      'メールアドレスとパスワードで新規登録します。初回に表示名・アイコン・所属店舗を決めます。所属店舗は必須で、複数選べます。そのあと、本部から申請承認権限を付与された管理者が許可するまで使えません。許可の手続きは「管理者通知」ルームにだけ届き、予約通知や店舗ルームの一般メンバーには出ません。許可されると、選んだ所属で閲覧だけできる状態です。メッセージの送信やグループ作成は、管理者が後から権限を付けます。',
      '所属店舗は、左上の自分のアイコンから「プロフィール設定」で後から変更できます。変更も管理者の許可が必要で、許可されるまで今の所属のままです。1対1トークは、所属店舗が同じ相手だけ始められます。',
      '自分のアイコンは、画面左上の自分のアイコンを押し、「アイコンを変更」から変更できます。',
      '本部から全権管理者として認定された利用者には、右上の自分のアイコンを押したメニューに「M-talk権限管理」が表示されます。一般利用者や通常のルーム管理者には表示されません。',
  ...
  },
  {
    ...
    content: [
      ...
      '自分の発言は、メニューの「編集」から本文を直せます。直す前の文章は取り消し線で残し、新しい文章の上に表示します。何をどう直したか後から見ても分かるようにしてあり、履歴は消せません。通知は送り直しません。画像・ファイル・カード・大きく送ったスタンプは編集できません。他人の発言は削除も編集もできません。',
    ].join('\n'),
  },
  {
    id: 'image-file',
  ...
  },
  {
    ...
    content: [
      ...
      '未参加のグループは一覧に出ても、ボタンから直接は入れません。「招待で参加」を押して届いたリンクを貼ると参加できます。',
    ].join('\n'),
  },
  {
  ...
  },
  {
    ...
    title: 'AIのWeb検索',
    keywords: [
      'web検索', 'ウェブ検索', 'ネット検索', 'ググ', '調べて', '検索して答える',
      '出典', '最新情報', 'ニュース', 'モデル', 'sonar', '料金', '課金',
    ],
    content: [
      '雑談AIは、ルーム設定で「AIのWeb検索」がONのときだけ、Webを検索して答えられます。既定はOFFです。',
      '設定は入力欄で「設定」と送ると開ける権限設定ページの「AI返信・検索」区分にあります。同じ場所で検索に使うモデル（標準 sonar／高精度 sonar-pro）も選べます。',
      '検索が動くのは「調べて」「ググって」のような明示的な依頼か、疑問形かつ最新情報・一般知識が要る質問のときだけです。ふだんの雑談や使い方の質問では検索しません。',
      'Webを見て答えたときは、回答の最後に出典のURLが付きます。検索1回ごとに料金が発生するため、使わないときはOFFに戻してください。',
      '売上・客数など店舗の実データはWeb検索の対象外です。正確な数字は入力欄の「＋」→「ジャーナルに聞く」で確認してください。',
    ].join('\n'),
  },
  {
    id: 'permissions',
    title: '権限・閲覧専用・利用停止',
    keywords: ['権限', '閲覧専用', '閲覧のみ', '送れない', '見られない', '使えない', '利用停止', '制限', '管理者', '承認', '許可', '不許可'],
    content: [
      'M-talkには全体の利用権限と、ルームごとの閲覧・送信・招待・管理権限があります。',
      '閲覧専用ルームでは「このルームは閲覧専用です。メッセージは送信できません」と表示されます。',
      '新規登録は、本部から申請承認権限を付与された管理者の許可が必要です。許可と不許可のカードは「管理者通知」ルームにだけ届き、予約通知や店舗ルームの一般メンバーには見えません。許可されると閲覧だけできる状態で始まります。送信やグループ作成は後から付けます。',
      'M-talk全体が停止または期限付き制限されている場合は、ログイン後の画面に理由と期限が表示されます。権限の変更はM-talk管理者が行います。',
      '本部が全権管理者として認定した人は、全店舗と全共有ルームへ自動で所属し、全店舗の予約・予定カレンダーを閲覧できます。個人間の1対1トークには自動参加しません。',
    ].join('\n'),
  },
  {
    id: 'history',
