...
export const EXPENSE_RECEIPT_PROMPT_ADDITION = [
  ...
  'JSONの receipt に追加で line_items（商品明細）を必ず含めること: "line_items":[{"name":"品名","price":"¥価格","rate":8}]（rate は消費税率 8 または 10）。例: [{"name":"お徳用おろし生姜","price":"¥299","rate":8},{"name":"食器用洗剤","price":"¥880","rate":10}]。明細は最大10件。読めない価格は null。',
  '【明細の読み方（スーパー/百貨店形式）】「JANコード等の数字だけの行」の次の行に「品名 …… ¥価格」が印字される形式が多い。数字だけの行は無視し、品名と同じ行の右端の ¥金額を price としてペアで拾うこと（末尾の「#」「*」等の記号は金額に含めない）。明細を途中で打ち切らず全品目を拾う。品名は印字どおり忠実に転記し、読みにくい文字を推測で別の単語に置き換えない（読めた部分だけでよい）。',
  '【税率記号】品名の先頭/末尾の「※」「*」「軽」は軽減税率8%対象の印。印のある品目は rate=8、無い品目はレシートの注記に従う（注記が無ければ飲食料品=8、それ以外=10）。',
  '金額は厳密に読み分ける: 小計(税抜)=net_sales、消費税額(外税/内税)=tax_amount、合計(税込・支払総額)=gross_sales。「お預り(預り金)」「お釣り」は支払総額ではないので gross_sales に絶対に入れない。',
  '【内税（税込）レシートの読み分け・最重要】「◯%税込 ¥X／うち税額 ¥Y」形式の場合: gross_sales=「合計」の支払総額、tax_amount=各税率の「うち税額」の合計（例: 8%のうち税額¥370＋10%のうち税額¥0 → "¥370"）、net_sales=gross_sales−tax_amount。「◯%税込 ¥X」の X は税込小計であり、絶対に net_sales や tax_amount に入れてはならない（例: 「10%税込 ¥5」の 5 を tax_amount にするのは誤り）。',
  // 仕入れ先店名（store_name）の正規化。経費カードの「仕入先」表示・記録に使われる。装飾ロゴの誤読対策。
  '【仕入れ先店名（store_name）は「レシート最上部の店名／ロゴ」から読む・最優先】store_name は必ずレシートの一番上にある店名（最上部のロゴ・社名）を採用すること。住所・地名・取扱商品名・「◯◯店」「◯◯支店」などを店名と取り違えない。最上部が装飾ロゴで読みにくい場合も、下記の既知仕入れ先の手がかりを最優先で適用する。',
  '【仕入れ先店名（store_name）の正規化・最優先で必ず適用】既知の仕入れ先は、上部ロゴが装飾フォントで読みにくくても、下記の手がかりが1つでも一致すれば store_name をその正式名称に確定すること（支店名や「店」は付けない）:',
  '・正式名称「クック-Y」… 手がかり（いずれか1つでOK）: 上部ロゴが「クック」＋「Y」のように見える ／ 適格請求書登録番号が "T5011401006431" ／ 電話番号が "03-5367-2825"。→ store_name は必ず "クック-Y" にする。装飾ロゴの "クック-Y" を「クッキー」と読み崩さないこと。「クッキー」「クッキー新宿」「クッキー◯◯」など “クッキー” を含む名称には絶対にしない。支店名・地名（「新宿」等）も付けない。',
].join('\n')
