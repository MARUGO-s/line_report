...
const EXPENSE_VENDOR_PROMPT_BLOCKS: ExpenseVendorPromptBlock[] = [
  ...
  },
  {
    ...
    rules: [
      ...
      '→ 【品名＝発送元（ご依頼主）・最重要】運送会社の代引領収証は中身の商品明細が無く、実質の仕入先は**荷物の発送元（ご依頼主＝送り主）**。発送元は伝票の**左下**に会社名・氏名で記載される（例「（有）広松養魚場」/「三代目 廣松 充」）。**store_name と line_items の品名は、この発送元（左下のご依頼主の会社名）を採用する**（例: store_name="（有）広松養魚場"、line_items=[{name:"（有）広松養魚場", …}]）。「お届け先」（claudia2 森野 様 等＝自店の宛先）や、手書きの「品名（なまもの/食品/早め）」欄を店名・品名にしないこと。発送元がどうしても読めないときだけ手書きの品名欄や"ヤマト運輸"を使う。',
