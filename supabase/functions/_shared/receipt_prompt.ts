...
export const STORE_RECEIPT_PROMPT_MAX_CHARS = 12000
...
const EXPENSE_VENDOR_PROMPT_BLOCKS: ExpenseVendorPromptBlock[] = [
  ...
  },
  {
    ...
    rules: [
      ...
      '→ 【税率＝品名行(1行目)の軽減税率マークで判定・最重要】軽減税率8%のマーク（「※」。OCRの揺れで ＊/*/★/◆/軽 等に見えても**すべて※と同等に扱う**）は**品名行(1行目)の先頭〜品名直前**にある。**マークあり→rate=8、無し→rate=10**。価格は2行目にあるが、税率は必ず直上(1行目)のマーク有無で決める（2行目の「外h/内#」や品名の意味で決めない。例「※冷凍食品 / 外h 900」→rate=8、「買物袋 / 内# 5」→rate=10）。COREの「飲食料品=8」推定に頼らず必ずマークで判定する。',
      '→ 【検算＝下部サマリーを正本アンカーに・全品10%の罠を名指しで弾く・最重要】下部「軽減8%課税 ◯／消費税等 ◯」「10%課税 ◯／消費税等 ◯」「(◯点) 合計 ◯円」を tax_breakdown・gross_sales に反映（各 {rate, total=課税額(税抜対象)＋消費税等, tax=消費税等}、gross_sales＝「合計◯円」の支払総額）。出力前に必ず照合する: ①rate=8の品のprice合計＝「軽減8%課税」、rate=10の品のprice合計＝「10%課税」を**税率別に**一致（例 8%群=7,368／10%群=5）。②**「軽減8%課税」が0より大きいのに rate=8 の品が0件なら、それは※の取りこぼし（全品10%バグ）。品名行のマークを総当たりで再走査し、軽減8%課税額に一致する品群を rate=8 に直す**。③件数＝「◯点」。①②③が合うまで出力しない。',
