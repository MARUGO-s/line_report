 ...
 *  仕入先ブロック(TOBU/MARUSHO/ダイエー…)がGroqに届かず無効化される（過去2000で TOBU/MARUSHO が
 *  丸ごと切られていた）。EXPENSE_RECEIPT_PROMPT_ADDITION の全長を必ず上回る値にすること。 */
export const STORE_RECEIPT_PROMPT_MAX_CHARS = 8000
...
const EXPENSE_VENDOR_PROMPT_BLOCKS: ExpenseVendorPromptBlock[] = [
  ...
  },
  {
    ...
    rules: [
      '発動条件（いずれか）: 上部ロゴが「MARUSHO」/「丸正」/「丸正総本店」 ／ 適格請求書登録番号が "T4011101009628" ／ 電話番号が "03-3357-4381" ／ 各品目の**行頭**が「外8」「外10」「内8」「内10」で始まるレイアウト。',
  ...
  },
  {
    ...
    rules: [
      '発動条件（排他的識別子のいずれか）: 適格請求書登録番号 "T3040001015833" ／ 電話番号 "03-3350-3504" ／ 最上部ロゴ「AEON」＋社名「株式会社イオンフードスタイル」／「ダイエー◯◯店」の印字。※ただし各品目の**行頭**が「外8/外10/内8/内10」で始まる、または上部ロゴが「MARUSHO」「丸正」のレシートには本ブロックを適用しない（それはMARUSHOブロックの対象）。脚注「※印は軽減税率8％対象商品」や「( N個 × 単◯◯ )」レイアウトは補助シグナルで、単独では確定にしない。',
