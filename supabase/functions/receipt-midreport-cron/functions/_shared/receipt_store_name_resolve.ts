...
export function normalizeStoreToken(value) {
  return String(value || '').toLowerCase().replace(/株式会社ワルツ/g, '').replace(/[^0-9a-zぁ-んァ-ヶ一-龠々]/g, '').trim();
}
/** レシートに英字で出るブランド。displayName は line_receipt_entries / 返信で使う表記に合わせる。 */ const RECEIPT_LATIN_BRAND_PROFILES = [
  {
...
function extractLatinLettersLower(text) {
  const m = String(text || '').toLowerCase().match(/[a-z]/g);
  return m ? m.join('') : '';
}
function sortedLatinFingerprint(latin) {
 ...
 */ function tryMatchReceiptLatinBrand(rawName) {
...
export function resolveBestStoreName(rawName) {
  ...
  const candidates = [
    ...MARUGO_GROUP_STORE_OPTIONS
      ...
      store,
      norm: normalizeStoreToken(store)
    })).filter((row)=>row.norm.length > 0).sort((a, b)=>b.norm.length - a.norm.length);
  ...
  /** 店舗一覧に一致しない生の OCR 表記は採用しない */ return null;
}
export function findBestStoreNameInText(text) {
