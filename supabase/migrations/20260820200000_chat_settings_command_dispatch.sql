-- 「設定」は店舗Botがいないルームでも、ルーム設定カードを返す。
...
begin
  ...
  v_is_settings := new.kind = 'text'
    and trim(new.content) in ('設定', '権限設定', 'せってい', 'ルーム設定');
  ...
  if (v_store_key is null or v_store_key = '') and not v_is_settings then
    return new;
  end if;
