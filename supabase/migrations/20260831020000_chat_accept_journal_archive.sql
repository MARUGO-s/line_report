-- M-talk の店舗ルームへ落とされた .lzh を受け取れるようにする。
--
-- 受けるだけで、保管と解析は /pos-journals/upload に任せる。chat-knowledge が
-- ルームから店舗を解決して転送し、結果カードを返す。
--
-- 汎用の添付として置きっぱなしにはしない。取込先の店舗が決まらないルームでは
-- クライアント側が電子ジャーナル画面へ誘導する。
