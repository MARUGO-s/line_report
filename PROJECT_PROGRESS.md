...
- Supabaseの実ログ（`foodcourt_qa_history`）で確認したところ、この質問に送られた`analysis_methods`は`mix,decompose,timing,bundle,event,margin,kpi,goal`の8種類全部で、直前のクロワッサン相談で選んだ手法がそのまま引き継がれていた。原因は`public/foodcourt-qa-planner.js`の重ね聞き処理（`skipMethods`）が、前回選んだ分析手法を「話題が変わったかどうか」を見ずに無条件で次の質問へ引き継いでいたこと。`目標・損益分岐・撤退`(kpi)・`粗利・採算`(margin)のいずれかが含まれていると、質問文の内容に関わらずサーバー側でKPI/採算試算（`analysisMethodsForceKpi`）が強制的に走る仕組みのため、無関係な質問にも前回の新商品トライアルの表がそのまま付いてしまっていた。
- 修正: 重ね聞きで前回の手法を引き継ぐ際、今回の質問自体が`wantsKpiTargets()`で改めてKPIを求めていない場合は、引き継ぐ手法から`kpi`・`margin`だけを外すようにした（他の軽い分析角度・売上構成・要因分解などはそのまま引き継いでよい）。今回の質問が改めてKPIを求めていれば、従来通り`kpi`が追加される。KPI試算は元々「この質問だけの例外」という設計方針だったが、重ね聞きの引き継ぎ処理だけがこれに従っていなかった。
- 回帰テスト追加（`foodcourt_qa_planner.test.mjs`: 無関係な重ね聞きでkpi/marginが外れること、改めてKPIを求めた質問では引き続き追加されること）。`test:foodcourt`269件成功。
- 本題の回答が空になる症状については、巨大化した無関係プロンプト（8手法分の知識パック＋新商品KPI試算の全文）が一因の可能性はあるが、外部AI呼び出しの実エラーは未確証。今回の修正で今後この規模の無関係プロンプトは発生しにくくなるはずだが、再発した場合は新しいログで別途調査する。

### 2026-09-21 - KPI試算前提の項目名をクロワッサン（焼成）専用語から商品名を問わない語へ変更

- 利用者から、新商品・KPIの試算前提の入力欄（「1回に焼ける個数」「1日の焼成回数」等）がクロワッサン（焼き物）にフォーカスしすぎており、今後様々な新メニューを検討するときにも包括的に使える項目名にしてほしいという要望があった。
- `kpi_scenario.ts`の`KPI_ASSUMPTION_LABELS`（KPI試算前提の正本ラベル）を軸に、`public/foodcourt.html`のQ&A入力欄、`public/jnm/jnl2txt.html`の店舗設定タブ（`KPI_ASSUMPTION_FIELDS`）、`journal_store_context.ts`のAI向け前提説明文、`foodcourt-qa-planner.js`の確認文まで一貫して用語を変更: 「1回に焼ける個数」→「1回の仕込みで作れる数量」、「1日の焼成回数(の上限)」→「1日の仕込み回数(の上限)」、「仕込み・焼成に割ける人員」→「仕込み・提供に割ける人員」、「1日の仕込み・焼成時間」→「1日の仕込み・提供時間」、「焼成能力」→「仕込み能力」。
- KPI試算エンジン内部で計算結果の説明に使う「焼成上限」という言葉も、`kpi_scenario.ts`・`foodcourt_kpi.ts`・`foodcourt_journal_detail.ts`・`FOODCOURT_KPI_POLICY`のすべてで「仕込み上限」に統一した。付録テーブルのセル表示（`◯個（観測◯個／仕込み上限◯個）`）もこの用語に合わせて更新。
- 内部のフィールド名（`bakeBatchUnits`等のキー）・保存済みDBの店舗設定値は変更していない。表示文字列（ラベル）だけの変更のため、既存の店舗ごとの入力値はそのまま有効。
- 回帰テスト更新（`foodcourt_kpi.test.ts`・`kpi_scenario.test.ts`・`journal_chat_query_planner.test.mjs`の期待ラベル文字列）。`test:foodcourt`180件、`journal-ai`関連(Deno 46件・Node 137件)すべて成功。キャッシュv31→v32。保存済み回答は再質問が必要。

### 2026-09-21 - 新商品KPI付録テーブルに観測数と焼成上限をセル内併記（表示の汎用改善）

- 利用者から、保守/標準/強気の「予想販売数/日」の差が小さく見える（54/55/57個）ことについて、表の中に観測値と上限を出せば「生産能力が需要に追いついていない」ことが表だけで分かる、という要望があった。あわせて、クロワッサンに限らず今後の様々な商品のKPI分析でも同じ表示になるよう、汎用的な実装・指示にしてほしいという要望も受けた。
- `foodcourt_kpi.ts`の`formatKpiUserAppendix()`で、焼成上限×廃棄控除に頭打ちしたシナリオの「予想販売数/日」セルを`54個（観測333.3個／焼成上限54個）`のように観測値（丸め前の類似商品アンカー）と上限を併記する形式に変更。表の上にあった「観測◯個→上限◯個」の個別列挙は、頭打ちの原因（需要でなく生産能力）を説明する1文に置き換え、詳細はセル側に一本化した。
- `FOODCOURT_KPI_POLICY`に、セルの括弧内表示がある場合は生産能力の頭打ちだと明記し、上限を上げる手段（バッチ回数・バッチ個数・廃棄率）を示す指示を追加。ロジック・指示のどちらも商品名に依存しない汎用実装（`row.outlookExceedsCapacity`等の既存の商品非依存フィールドを使用）で、対象商品を問わず適用される。
- 回帰テスト追加（頭打ち時のセル形式、頭打ちが無い場合はセルが素の個数のままであること、ポリシー文が商品名に依存しないこと）。`test:foodcourt`180件成功。キャッシュv30→v31。保存済み回答は再質問が必要。

### 2026-09-21 - フードコートQ&Aの分析手法チップでページが上へ戻る不具合を修正

- 分析手法チップ（複数選択可）を選ぶたびに、`askQuestion()`が再描画後に`dom.qInput.focus()`を呼んでおり、ブラウザの自動スクロールで画面が入力欄の位置まで戻ってしまっていた。
- `public/foodcourt.html`のQ&A入力欄に関する`focus()`呼び出し5箇所すべてを`focus({preventScroll:true})`に変更。回帰テスト追加、`test:foodcourt`179件成功。DB・API・AI挙動の変更なし（画面のみ）。

### 2026-09-21 - 新商品KPI試算に「前提の妥当性チェック」を追加し反証AIの担当を拡張（重要）

- 利用者から、KPI試算に推測（仮定シナリオ）が増えてきたため、過大評価・過小評価をどこかのAIが必ず確認する体制にしてほしいと要望があった。現状のAI構成を調査したところ、反証AI④（品質管理担当）は「新しい施策のKPI試算は後段のサーバー確定ブロックが担当する」として、KPI試算の中身を一切見ていなかった。数値監査（コード）も「回答の数字が提供データに実在するか」だけを見ており、前提（仮定）自体の妥当性は誰もチェックしていなかった。
- `foodcourt_journal_detail.ts`に`assessFoodCourtNewItemPlausibility()`を追加。新商品の想定販売数（焼成上限で頭打ち済み）を、店内で最も個数の出ている既存商品の実績日次個数と機械的に比較し、想定が既存の最多販売商品の実績以上なら「過大評価の可能性」を立てる。比較は当店の実測値どうしであり、業界目安は使わない。
- 反証AI④のシステムプロンプトを更新し、算術の再計算は引き続き担当外としつつ、この比較結果（短い参考ブロックのみ、詳細なKPI表は渡さない）を必ず確認し、過大/過小評価の可能性を指摘する担当を新設。`FOODCOURT_KPI_POLICY`・KPI知識パック（doc+runtime）にも同様の指示を追加。
- 回帰テスト追加（比較の算出値、ユーザー向け注釈、反証AIへの実際の伝達内容とKPI詳細表が漏れないことを確認）。`test:foodcourt`178件成功。キャッシュv29→v30。設計の正本は`docs/フードコートAI分析システム_設計解説.md`の反証AI④節。
