...
- 予測を全面禁止していた4か所（Journal側 `jnl2txt.html` / `ai-analyze`、フードコート側 `foodcourt_compare.ts` の施策書式と統合AI (5-2)）を「根拠のない断定的な予測は避ける。ただし前提を明示したシナリオ試算は出す」へ書き換え。フードコートAIキャッシュは v19 → v20。
...
- 検証: `tests/kpi_scenario.test.ts` 14件を新規追加（粗利率・損益分岐・3シナリオ別売上・目標個数・撤退ライン・入力値変更で結果が動くこと）。`journal_chat_query_planner` 87件、foodcourt 115件、構造・セキュリティ 67件、`journal_ai_orchestrate` 12件が成功。`@electric-sql/pglite` と `graphify-out/` を要する既存テストは当環境の未導入依存により未実行。
