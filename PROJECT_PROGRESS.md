...
- Implementation: `public/pos-journal.html`を指定順の5タブへ再編し、初期表示も売上サマリーにした。KPI・日別推移・決済内訳・日別サマリーは売上サマリー、AI分析・質問・PDF・履歴はAI売上分析へ分離した。AI売上分析には、現在の画面を残してJournal Reportの詳細分析を別タブで開く「詳しく分析する」ボタン（`/jnm/jnl2txt.html`）を追加。既存のアップロード、保管原本のダウンロード／削除、AI分析・質問・PDF・履歴のID・イベント・APIは維持。タブ選択はlocalStorageに保存し、左右矢印／Home／Endのキーボード操作にも対応。
