...
- Production E2E: `5b06d48` / `d6e2837` をmainへpushし、`admin-api` version 845（ACTIVE / `verify_jwt=false`）へ反映。既存LZH `101520260129220406670001.lzh` を10分限定・`bistrocavacava`店舗スコープの一時セッションで重複再送し、HTTP 200、`uploaded=0 / repaired=0 / duplicate=1 / failed=0`、保存レポート`created=0 / updated=2`を確認。既存ID `1785441706573_0_xv9o6i` / `1785441706574_1_ydz3ud` と作成日時を維持し、両方を21営業日・64会計・¥1,087,600・128名・64組へ復元。原本は3件、単月レポートは2件のまま、両HTMLオブジェクトも`pos-report-html`に存在。一時セッションは即時削除し有効行0、未認証APIは401。ActionsはEdge run `31664341526`、Pages run `31657791569`がsuccess。公開`pos-journal.html`はHTTP 200で自動作成表示を確認し、`解凍変換ソフト`のWebファイル9点は正本とSHA-256一致。
