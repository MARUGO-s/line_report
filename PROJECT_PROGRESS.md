...
- Implementation: `public/pos-journal.html`を指定順の5タブへ再編し、ページを開くたび必ず売上サマリーを初期表示にした。KPI・日別推移・決済内訳・日別サマリーは売上サマリー、AI分析・質問・PDF・履歴はAI売上分析へ分離した。AI売上分析には、現在の画面を残してJournal Reportの詳細分析を別タブで開く「詳しく分析する」ボタン（`/jnm/jnl2txt.html`）を追加。商品ランキングは従来の合計に加え、商品コードまたは保存済みのカテゴリを基準にフード／ドリンクだけを切り替えて表示できる3表示にした。既存のアップロード、保管原本のダウンロード／削除、AI分析・質問・PDF・履歴のID・イベント・APIは維持。左右矢印／Home／Endのキーボード操作にも対応。
