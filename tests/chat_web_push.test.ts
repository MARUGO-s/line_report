...
test("chat PWA registers a service worker and lets the signed-in user enable notifications", async () => {
  ...
  assert.match(html, /async function subscribePushPreferenceChanges/)
  assert.match(html, /dropRealtimeChannel/)
  assert.match(html, /chat-push-preference-\$\{currentUser\.id\}-\$\{Date\.now\(\)\}/)
  assert.match(html, /if \(state === 'joined' \|\| state === 'joining'\) return/)
  assert.match(html, /Push preference realtime error/)
  ...
  assert.doesNotMatch(html, /armPushRestoreOnUserGesture/)
  ...
  assert.match(serviceWorker, /line-report-chat-v59/)
  ...
  assert.doesNotMatch(html, /if \(currentGroupId\) delete counts\[currentGroupId\]/)
  assert.match(html, /const \{ error \} = await sb\.from\('chat_read_states'\)\.upsert/)
  assert.match(html, /if \(error\) throw error;[\s\S]*?finally \{[\s\S]*?await loadUnread\(\)/)
  assert.match(html, /if \(!document\.hidden\) markGroupRead\(currentGroupId\);[\s\S]*?else loadUnread\(\)/)
  ...
  assert.match(html, /data-action="reply"[^>]*>[\s\S]*?msg-menu-action-icon">↩<\/span><span>返信/)
  ...
  assert.match(html, /\.select\('group_id, chat_users\(is_bot\)'\)/)
  assert.match(html, /function roomBotMarkHtml\(group\)/)
  assert.match(html, /botRoomIds\.has\(Number\(group\.id\)\)/)
  assert.match(html, /class="room-bot-mark"[^>]*>BOT<\/span>/)
  assert.match(html, /talk-list-count/)
  assert.match(html, /is_store_room/)
  assert.match(html, /店舗固定ルームは退出・削除できません/)
  assert.match(html, /id="talkCtxPurge"/)
  assert.match(html, /function purgeTalk/)
  assert.match(html, /chat-room-purge/)
  ...
  assert.match(html, /function openSearchLauncher/)
  assert.match(html, /searchLauncherOverlay/)
...
test("chat push endpoint requires Supabase Auth and excludes the sender", async () => {
  ...
  assert.match(edge, /async function hasActiveChatAccess/)
  assert.match(edge, /\.from\("chat_user_access"\)/)
  assert.match(edge, /\.eq\("access_enabled", true\)/)
  assert.match(edge, /\.is\("deleted_at", null\)/)
  assert.match(edge, /restricted_until\.is\.null,restricted_until\.lte/)
  ...
  assert.match(edge, /\.in\("user_id", memberRecipientIds\)/)
  assert.match(edge, /recipient access load failed/)
  assert.match(edge, /memberRecipientIds\.filter\(\(id\) => activeRecipientIds\.has\(id\)\)/)
