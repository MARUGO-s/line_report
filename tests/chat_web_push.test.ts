...
test("chat PWA registers a service worker and lets the signed-in user enable notifications", async () => {
  ...
  assert.match(serviceWorker, /line-report-chat-v39/)
  ...
  assert.match(html, /body\.chat-open \{[\s\S]*?--mobile-safe-bottom: 0px;/)
  assert.match(html, /body\.chat-open \{[\s\S]*?padding-bottom: 0;/)
  assert.match(html, /body\.chat-open \.input-area \{/)
  assert.match(html, /--chat-composer-bottom/)
  assert.match(html, /function measureSafeAreaBottomPx/)
  assert.match(html, /measureSafeAreaBottomPx\(\) \/ 3/)
