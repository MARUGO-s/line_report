...
test('foodcourt boost finalizer uses a fixed trust policy and skips duplicate external orchestration', async () => {
  ...
  assert.match(ai, /フードコート売上の期間不足や税区分差だけを理由に、同期間のイベント・競合・順位・客層の事実まで捨ててはいけません/);
  ...
  assert.match(ai, /expected_day_count:\s*boundedNonNegativeInteger/);
  assert.match(ai, /missing_dates:/);
  assert.match(ai, /sales_basis:/);
  assert.match(ai, /baseline:\s*\{/);
