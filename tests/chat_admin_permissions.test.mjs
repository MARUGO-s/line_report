...
test("dedicated admin API uses chat-only soft removal and exposes direct-room controls", async () => {
  ...
  assert.match(api, /authResult\.scopeKind === MTALK_ADMIN_SCOPE/)
  assert.match(api, /headquartersChatAdminAuthority\(\)/)
  assert.match(api, /loadMtalkAdminAuthority\(supabase, session\.metadata\)/)
  assert.match(api, /この権限はM-talk管理だけに利用できます/)
  ...
  assert.match(api, /status:\s*conflict\s*\?\s*409\s*:\s*forbidden\s*\?\s*403\s*:\s*400/)
  ...
  assert.match(adminUi, /Botの削除・復元はこの管理画面だけ/)
  assert.match(adminUi, /data-remove-bot/)
  assert.match(adminUi, /data-restore-bot/)
  ...
  assert.match(chat, /const CHAT_ACCESS_COLUMNS = 'user_id, access_enabled, can_start_direct, can_create_group, can_browse_users, default_can_send, signup_status, restriction_reason, restricted_until, deleted_at'/)
