...
test("reservation cache schema, cron time, and protected hybrid route are wired", async () => {
  ...
  assert.match(cron, /from "\.\.\/_shared\/internal_cron_auth\.ts"/)
  assert.match(cron, /if \(!\(await isInternalCronAuthorized\(req, supabase\)\)\)[\s\S]{0,120}401/)
  assert.match(cron, /const bearer = extractBearerToken\(req\)/)
  assert.match(cron, /"Authorization": `Bearer \$\{bearer\}`/)
