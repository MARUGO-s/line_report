...
test("schedule page accepts group and group_id, and chat keeps the room when opening the card", async () => {
  ...
  assert.match(chat, /openReservationSchedule\(schedule.groupId, schedule.tab, schedule.month\)/)
  assert.match(chat, /resolveMtalkCardScheduleLink\(cardLink.getAttribute\('href'\), currentGroupId\)/)
  assert.match(chat, /source.origin === candidate.origin/)
  assert.match(html, /params.get\('month'\)/)
})
