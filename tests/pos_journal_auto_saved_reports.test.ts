...
Deno.test("mixed payment amounts survive auto report and shared-day round trip", () => {
  const day = sampleDay("2026-08-29", 1_000);
  day.groups = 1;
  day.guests = 1;
  day.receipts = [{
    no: "mixed",
    ...
    total: 1_000,
    guests: 1,
    payment_breakdown: { "現金": 500, "QRコード": 500 },
    items: [{
      code: "0000000000101",
      name: "混合決済商品",
      unit: 1_000,
  ...
  assertEquals(monthly.data.sales[0].payments.byMethod, {
    "現金": 500,
    "QRコード": 500,
  });
  assertEquals(monthly.data.posJournalDays[0].payment_breakdown, {
    "現金": { count: 1, amount: 500 },
    "QRコード": { count: 1, amount: 500 },
  });
  const restored = buildPosJournalDaysFromSavedReports([{
    id: monthly.id,
    data: monthly.data,
    created_at: "2026-08-30T00:00:00Z",
  }], "2026-08");
  assertEquals(restored[0].receipts[0].payment_breakdown, {
    "現金": 500,
    "QRコード": 500,
  });
  assertEquals(restored[0].payment_breakdown, {
    "現金": { count: 1, amount: 500 },
    "QRコード": { count: 1, amount: 500 },
  });
});
